...
//!
//! Plugins from project directories (`.igrok/plugins/`, `.claude/plugins/`)
...
//!
//! **Trust storage**: `~/.igrok/trusted-plugins` (one canonical path per line).
//!
...
/// Name of the trust-store file under `~/.igrok/`.
...
impl TrustStore {
    ...
    ///
    /// If `~/.igrok/trusted-plugins` does not exist, returns an empty store.
    /// If the file cannot be read, logs a warning and returns an empty store.
    pub fn load() -> Self {
        // Gate on user_grok_home() so a project's `.igrok/trusted-plugins` is never
    ...
    ///
    /// Canonicalizes the path and appends it to `~/.igrok/trusted-plugins`.
    ...
    /// rewrites `~/.igrok/trusted-plugins` without the revoked entry.
    ...
    /// trust via `~/.igrok/trusted-plugins`.
