...
//!
//! Searches from cwd to repo root, plus `~/.igrok/`. Also discovers
//! `*.md` files in rules directories: vendor-prefixed `.igrok/rules/`,
...
//! (`~/.igrok/rules/`, `~/.claude/rules/`, `~/.cursor/rules/`).
...
/// Find `*.md` files in `.igrok/rules/`, `.claude/rules/`, and `.cursor/rules/`,
...
/// Read Agents.md from ~/.igrok/, git repo root, and session cwd.
...
mod tests {
    ...
    async fn home_and_project_rules_have_stable_order_without_doubled_paths() {
        ...
        fs::create_dir_all(repo.join(".igrok/rules")).unwrap();
        ...
        for (path, content) in [
            ...
            (repo.join(".igrok/rules/a.md"), "repo-grok"),
        ...
        for path in [
            grok_home.join(".igrok/rules/doubled.md"),
    ...
    async fn nested_grok_home_keeps_project_role_in_repo_order() {
        ...
        fs::create_dir_all(nested.join(".igrok/rules")).unwrap();
        init_git_repo(&repo);
        ...
        fs::write(nested.join(".igrok/rules/project.md"), "nested-project-rule").unwrap();
    ...
    async fn overlapping_grok_home_and_project_root_merges_roles() {
        ...
        fs::write(repo.join(".igrok/rules/project.md"), "project-grok-rule").unwrap();
        fs::write(repo.join(".claude/rules/project.md"), "project-claude-rule").unwrap();
        fs::create_dir_all(repo.join(".igrok/.igrok/rules")).unwrap();
        fs::write(repo.join(".igrok/.igrok/rules/doubled.md"), "doubled").unwrap();
    ...
    async fn rule_frontmatter_is_stripped_but_named_frontmatter_is_preserved() {
        ...
        for (path, body) in [
            ...
            (repo.join(".igrok/rules/project.md"), "grok-project-body"),
