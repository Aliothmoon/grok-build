...
//! `~/.igrok/memory/{project-slug}-{hash8}/` to avoid polluting the user's repo.
...
/// `~/.igrok/memory/`. Workspace-scoped files live under a directory
/// named `{project-slug}-{hash8}`, e.g. `~/.igrok/memory/xai-a3f7b2c9/`.
#[derive(Debug, Clone)]
pub struct MemoryStorage {
    /// `~/.igrok/memory/`
    global_dir: PathBuf,
    /// `~/.igrok/memory/{project-slug}-{hash8}/`
...
impl MemoryStorage {
    /// Create a new `MemoryStorage` rooted at `~/.igrok/memory/`.
    ///
    ...
    ///
    /// File path: `~/.igrok/memory/{project}-{hash8}/sessions/YYYY-MM-DD-{slug}-{sid8}.md`
    ///
    /// - `date`: e.g. `"2026-02-23"`
