...
/// `~/.igrok/last-copy.txt` (grok's per-user home — short, stable, and
...
/// prefix collapses to `~/.igrok` (or `$GROK_HOME` when overridden), and a
...
mod tests {
    ...
    /// Without `GROK_COPY_FILE`, the default is `~/.igrok/last-copy.txt`
    ...
    fn display_copy_path_abbreviates_home() {
        if std::env::var_os("GROK_HOME").is_none() {
            ...
            assert_eq!(
                display_copy_path(&home.join(".igrok").join("last-copy.txt")),
                "~/.igrok/last-copy.txt"
            );
        }
