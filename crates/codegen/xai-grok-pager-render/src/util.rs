...
/// `~/.igrok` or `$GROK_HOME`, decided by the resolved home rather than by
...
/// User-facing path under [`grok_home()`], e.g. ``~/.igrok/config.toml``.
...
/// True when `path` is under user [`grok_home()`] (not project `{cwd}/.igrok`).
...
mod tests {
    ...
    fn display_grok_home_prefix_default_install() {
        ...
        assert_eq!(display_grok_home_prefix(), "~/.igrok");
    }
    ...
    fn display_user_grok_path_joins_relative() {
        ...
        assert!(path.contains(".igrok") || path.contains("$GROK_HOME"));
    }
    ...
    fn abbreviate_path_uses_home_when_under_default_grok() {
        if let Ok(home) = std::env::var("HOME") {
            ...
            let full = format!("{home}/.igrok/memory/MEMORY.md");
