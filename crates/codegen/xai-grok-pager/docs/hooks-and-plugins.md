...
- **Global hooks** — from `~/.igrok/hooks/`
- **Project hooks** — from `.igrok/hooks/` in your repo
...
2. **settings.json** — `extraKnownMarketplaces` from `~/.igrok/settings.json` or `~/.claude/settings.json`
...
Sources land in `~/.igrok/config.toml`:
...
Or in `~/.igrok/settings.json` / `~/.claude/settings.json`:
