...
5. **config.toml** (`~/.igrok/config.toml`)
...
Location: `~/.igrok/config.toml`. If the file is missing, Grok uses its built-in defaults, so you only need to set the values you want to override.
...
MCP servers can also be set per-project in `.igrok/config.toml`. Project-scoped config contributes `[mcp_servers]`, `[plugins]`, and `[permission]` rules; every other section loads only from `~/.igrok/config.toml`.

Priority for `[mcp_servers]` and `[plugins]`: `.igrok/config.toml` (current dir) > `<repo-root>/.igrok/config.toml` > `~/.igrok/config.toml`. `[permission]` rules aren't overridden by priority — they merge across all files with `deny` > `ask` > `allow` (see [22-permissions-and-safety.md](22-permissions-and-safety.md)).
...
Background workflows — the `workflow` tool, named `.igrok/workflows/*.rhai` scripts, `/deep-research`, and `/workflow` launches — are **on by default**. Disable with config, env, or remote settings.
...
Project workflows are discovered from `<repo-root>/.igrok/workflows/`; user workflows from `~/.igrok/workflows/`. Discovery and invocation key off the script's `meta.name`, so keep each filename aligned with its `meta.name`. Built-ins win over project names, and project names win over user names, so keep names unique across scopes.
...
`[hints]` is read from the **effective config merge**, with the usual precedence: system managed → user `managed_config.toml` → user `config.toml` → user `requirements.toml` → system `requirements.toml`, higher layers winning. The TUI only ever **writes** these to your user `~/.igrok/config.toml`.
...
Location: `~/.igrok/pager.toml`. This controls the TUI's look and feel. Changes apply on restart.
...
| Variable | Description |
|----------|-------------|
| `GROK_HOME` | Override config directory (default: `~/.igrok`) |
...
| Path | Description |
|------|-------------|
| `~/.igrok/config.toml` | Main configuration file |
| `~/.igrok/pager.toml` | TUI appearance configuration |
| `~/.igrok/auth.json` | Authentication credentials (auto-managed) |
| `~/.igrok/sessions/` | Persisted sessions (organized by working directory) |
| `~/.igrok/memory/` | Cross-session memory files and index |
| `~/.igrok/skills/` | User-scoped skill definitions |
| `~/.igrok/plugins/` | User-scoped plugins |
| `~/.igrok/agents/` | User-scoped agent definitions |
| `~/.igrok/lsp.json` | LSP server configuration (user-scoped) |
| `~/.igrok/logs/` | Internal log files (e.g. `unified.jsonl`, MCP server logs) |
| `.igrok/config.toml` | Project-scoped MCP servers, plugins, and permission rules |
| `.igrok/skills/` | Project-scoped skill definitions |
| `.igrok/plugins/` | Project-scoped plugins |
| `.igrok/agents/` | Project-scoped agent definitions |
| `.igrok/hooks/` | Project-scoped hooks |
| `.igrok/lsp.json` | LSP server configuration |

---

## Project-scoped configuration

Some settings can be set per-project by placing files in `.igrok/` inside your repository:

| File | What it configures |
|------|--------------------|
| `.igrok/config.toml` | MCP servers, plugins, permission rules, and the `[mcp] max_output_bytes` tool-result cap (other sections load only from `~/.igrok/config.toml`) |
| `.igrok/skills/` | Project-specific skills |
| `.igrok/hooks/` | Project-specific lifecycle hooks |
| `.igrok/agents/` | Project-specific agent definitions |
| `.igrok/lsp.json` | LSP server configuration |
| `.igrok/sandbox.toml` | Custom sandbox profiles |
...
|--------|----------|-------|
| User | `~/.igrok/lsp.json` | All projects |
| Project | `.igrok/lsp.json` | Current repository |
...
1. **Project** — `.igrok/lsp.json`
2. **User** — `~/.igrok/lsp.json`
