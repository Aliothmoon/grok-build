...
MCP servers are configured in `~/.igrok/config.toml` under `[mcp_servers.<name>]` sections.
...
> - `config.toml` — user-level (`~/.igrok/config.toml`) **or repo-level**
>   (`.igrok/config.toml` anywhere on the cwd → git-root chain; the deepest
...
> ```
>
> Precedence: requirements.toml > env > repo `.igrok/config.toml` >
...
By default `grok mcp add` writes to `~/.igrok/config.toml` (`--scope user`). Use `--scope project` to write to `.igrok/config.toml` in the current directory instead, which can be committed and shared with your team (see [Project-Scoped MCP Servers](#project-scoped-mcp-servers)). Header and environment variable values are stored verbatim, so reference secrets as `${VAR}` instead of pasting them into a committed project config (see [Example Configurations](#example-configurations)). `grok mcp list` shows servers from both scopes, marking project-scoped ones with `(project)` and disabled ones with `(disabled)`.
...
`grok mcp enable` / `disable` persist the personal on/off state to user `~/.igrok/config.toml` (`disabled_mcp_servers`, and `[mcp_servers.<name>].enabled` when that entry exists). Scope:
...
MCP servers can be configured per-project by placing a `.igrok/config.toml` in your repository:
...
Grok walks from the current directory up to the git repo root, loading `.igrok/config.toml` at each level:

| Location | Scope | Priority |
|----------|-------|----------|
| `~/.igrok/config.toml` | All projects | Lowest |
| `<repo-root>/.igrok/config.toml` | This repository | Medium |
| `<cwd>/.igrok/config.toml` | Current directory | Highest |
...
Project-scoped files contribute `[mcp_servers]`, `[plugins]`, and `[permission]` entries. Grok reads most other config sections only from `~/.igrok/config.toml`.
...
|--------|--------|----------|-------------|
| `config.toml` | Native Grok config | `~/.igrok/config.toml`, `.igrok/config.toml` | Always on |
...
For stdio servers, Grok captures the process's standard error to `~/.igrok/logs/mcp/<server>.stderr.log`, truncated on each launch. Check this file when a server starts but fails to handshake:

```bash
tail -f ~/.igrok/logs/mcp/filesystem.stderr.log
```
