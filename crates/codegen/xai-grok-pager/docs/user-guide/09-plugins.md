...
Place this file at `~/.igrok/settings.json` or `~/.claude/settings.json`.
...
Set these in `~/.igrok/config.toml`:
...
To hide the plugins and hooks interface entirely, set `disable_plugins = true` in `~/.igrok/pager.toml`.
...
For a hands-off install with no per-person step, also place the plugin's files where Grok discovers and trusts them automatically: `~/.igrok/plugins/`, or a directory your device-management tool manages that you point to with `[plugins].paths`. Then enable them with `[plugins].enabled`.
...
**A plugin's hooks or MCP servers don't run.** They stay inactive until the plugin is trusted. Reinstall with `--trust`, or place the plugin under `~/.igrok/plugins/` (auto-trusted). See [Trust and security](#trust-and-security).
...
| `.igrok/plugins/` | Project, shared through version control | Requires trust |
| `~/.igrok/plugins/` | User, every project | Trusted automatically |
