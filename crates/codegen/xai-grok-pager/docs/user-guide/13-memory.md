...
Memory is stored as Markdown files under `~/.igrok/memory/`:

| Location | Scope | Description |
|----------|-------|-------------|
| `~/.igrok/memory/MEMORY.md` | Global | Facts that apply across all your projects |
| `~/.igrok/memory/<project-slug>-<hash8>/MEMORY.md` | Workspace | Project-specific conventions and context |
| `~/.igrok/memory/<project-slug>-<hash8>/sessions/` | Sessions | Per-session summaries and logs |
...
Ask Grok to remember something, and it appends the note to a `MEMORY.md` file -- the workspace file for project-specific items, or the global `~/.igrok/memory/MEMORY.md` for cross-project preferences:
...
Forget is best-effort: the model searches memory and removes entries that match. For guaranteed removal, edit the files under `~/.igrok/memory/` directly and delete the entry yourself. To locate a file, open the `/memory` browser and press `y` to copy its path.
...
You can edit memory files directly under `~/.igrok/memory/`. The file watcher reindexes your changes on the next memory search. Use `/flush` to save the current session now, and `/dream` to consolidate session logs into organized topics.
...
```
Memory saved to ~/.igrok/memory/MEMORY.md
```
...
To edit memory from the shell, open the files in your editor directly -- for example, `$EDITOR ~/.igrok/memory/MEMORY.md`.
...
| `watcher.enabled` | `true` | Watch `~/.igrok/memory/` for external edits and reindex |
...
By default, Grok watches `~/.igrok/memory/` for external file changes. If you edit memory files directly (e.g., in your editor), the changes are picked up automatically on the next memory search:
...
```bash
ls ~/.igrok/memory/
cat ~/.igrok/memory/MEMORY.md
$EDITOR ~/.igrok/memory/MEMORY.md
```
