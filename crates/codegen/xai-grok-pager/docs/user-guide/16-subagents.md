...
| **How you set them** | At startup, or with agent definitions (`.md` files in `.igrok/agents/` or `~/.igrok/agents/`) | In `config.toml` (`[subagents.personas]`) or `.toml` files under `.igrok/personas/`; applied during subagent resolution |
...
- `.igrok/personas/*.toml` (project)
- `~/.igrok/personas/*.toml` (user)
...
model = "grok-build"
prompt_file = ".igrok/prompts/researcher.md"
```
...
# instructions_file = ".igrok/personas/concise.md"  # or load from a file
```

Grok Build also discovers roles from `.igrok/roles/*.toml` and personas from `.igrok/personas/*.toml`. Inline `config.toml` definitions take precedence over files.
