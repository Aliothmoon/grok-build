...
Sessions are identified by a unique session ID (a UUIDv7 when Grok generates it; a client may supply its own ID with `-s`) and stored on disk under `~/.igrok/sessions/`. Set `GROK_HOME` to override the base directory; when it is unset, Grok uses `~/.igrok`.
...
```
~/.igrok/sessions/<encoded-cwd>/<session-id>/
...
`grok du` (alias: `grok disk-usage`) reports what the grok home (`~/.igrok`) uses on disk. It lists each top-level directory, largest first, then each worktree with its size, type, age, label, and path. Worktrees the registry does not track appear as `untracked`. Pass `--json` for the same report as machine-readable output.

```text
Disk usage for ~/.igrok
    412.3 GB  worktrees
      1.2 GB  sessions
...
Worktrees
    ...
    380.0 GB  session             12d ago    my-fix ~/.igrok/worktrees/xai/worktree-abc
     32.3 GB  untracked (session) 40d ago           ~/.igrok/worktrees/xai/worktree-old
