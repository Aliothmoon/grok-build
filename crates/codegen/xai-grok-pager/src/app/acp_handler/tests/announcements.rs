#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    /// layer-injected seam so the developer's real `~/.igrok` cannot leak in.
    #[test]
    ...
    /// through the layer-injected seam (no real `~/.igrok` reads).
    #[test]
    ...
    /// layer-injected seam (no real `~/.igrok` reads).
    #[test]
