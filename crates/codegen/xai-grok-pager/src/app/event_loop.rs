...
pub(crate) async fn run(
    ...
    // Apply initial config (may come from existing ~/.igrok/pager.toml).
    ...
    loop {
        ...
        tokio::select! {
            ...
            Ok(()) = config_watcher.changed() => {
                ...
                // Preserve fields persisted via `~/.igrok/config.toml [ui]`
                // rather than `~/.igrok/pager.toml`. The watcher only knows
