//! Parse marketplace sources from `~/.igrok/config.toml`.
//!
//! Expected format:
//! ```toml
//! [[marketplace.sources]]
...
/// and `known_marketplaces.json` files under `~/.igrok/` and `~/.claude/`.
...
/// instead of `~/.igrok`/`~/.claude`. Each root is checked for
