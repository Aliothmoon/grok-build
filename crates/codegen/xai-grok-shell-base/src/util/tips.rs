...
//! persisted to `~/.igrok/tip_cursor.json`.
...
/// Load the cursor from `~/.igrok/tip_cursor.json`. Returns 0 on any error.
...
/// Save the cursor to `~/.igrok/tip_cursor.json`. Silently ignores write errors.
...
/// cursor in `~/.igrok/tip_cursor.json`, so every session sees the next tip
