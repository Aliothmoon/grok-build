...
pub enum ConfigUpdate {
    ...
    ///
    /// 1. The global `[mcp_servers]` table in `~/.igrok/config.toml`
    ///    changed.
    ...
    ///
    /// Project-scoped changes (`<cwd>/.igrok/config.toml`,
    ...
    /// (`<cwd>/.igrok/config.toml`, `<cwd>/.mcp.json`, or
    ...
    ProjectMcpServersChanged {
        /// The project root whose `.igrok/`, `.mcp.json`, or
    ...
    ModelsChanged,
    /// `~/.igrok/models_cache.json` was rewritten on disk (possibly by another
...
impl ConfigReloader {
    ...
    pub(crate) async fn run(
        ...
        loop {
            ...
            // same tick (e.g. `~/.igrok/config.toml` AND
    ...
    fn reload_config(&mut self) -> anyhow::Result<()> {
        ...
        // config (`~/.igrok/config.toml`) via toml::Value. Project-
        // scoped changes (`<cwd>/.igrok/config.toml`,
...
/// |----------------------------|-------------------------|-------------------|
/// | `ProjectConfigChanged`     | `<cwd>/.igrok/config.toml` | `<cwd>`           |
...
/// for `.igrok/config.toml`, `find_mcp_json_files` for `.mcp.json`) so
...
/// (`~/.igrok/config.toml`, `~/.claude.json`, `~/.cursor/mcp.json`)
...
mod tests {
    ...
    /// The hash must reflect ancestor `.igrok/config.toml` and `.mcp.json`
    ...
    fn hash_project_mcp_config_covers_ancestors() {
        ...
        std::fs::write(tmp.path().join(".igrok").join("config.toml"), "x = 1").unwrap();
        let h3 = hash_project_mcp_config(&child).expect("readable");
        assert_ne!(
            h2, h3,
            "ancestor .igrok/config.toml create must change the hash"
        );
    }
    ...
    fn parse_skills_config_with_paths() {
        let config: toml::Value = toml::from_str(
            r#"
[skills]
paths = ["/home/user/.igrok/skills"]
ignore = ["/tmp"]
"#,
        )
        .unwrap();
        let skills = parse_skills_config(&config);
        assert_eq!(skills.paths, vec!["/home/user/.igrok/skills".to_string()]);
...
"#,
    ...
    /// `ProjectConfigChanged` (`<cwd>/.igrok/config.toml`) and
    ...
    fn collect_project_cwds_excludes_home_claude_json() {
        let batch = vec![
            ...
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/x/.igrok/config.toml"),
            },
        ];
    ...
    fn collect_project_cwds_dedupes_and_extracts() {
        let batch = vec![
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/a/.igrok/config.toml"),
            },
            ConfigChangeEvent::McpConfigChanged {
                path: PathBuf::from("/repo/a/.mcp.json"),
            },
            ConfigChangeEvent::ProjectConfigChanged {
                path: PathBuf::from("/repo/b/.igrok/config.toml"),
            },
        ];
        let cwds = collect_project_cwds(&batch);
        assert_eq!(
