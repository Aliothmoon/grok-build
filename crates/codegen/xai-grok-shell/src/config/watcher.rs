...
pub enum ConfigChangeEvent {
    AuthChanged,
    GlobalConfigChanged,
    /// `~/.igrok/models_cache.json` changed — the on-disk `/v1/models`
    /// catalog cache was rewritten, possibly by **another** grok process
    /// sharing the same `~/.igrok` (the writer may also be this process;
...
/// Watches `~/.igrok/` for `auth.json`, `config.toml`, and `models_cache.json`
/// changes, plus any extra paths (project `.igrok/config.toml`, `.mcp.json`,
...
/// `<cwd>/.igrok/` (catches `<cwd>/.igrok/config.toml`). Recursing on `<cwd>`
...
impl ConfigFileWatcher {
    ...
    /// `<cwd>/.igrok/`. Use [`Self::watch_path`] later to register additional
    ...
    pub fn start(
        ...
        // includes `<cwd>/.igrok/config.toml` so the loop above
        // watches `<cwd>/.igrok/`), the call below installs a
    ...
    /// Register `<cwd>/` and `<cwd>/.igrok/` as **non-recursive** watch
    ...
    /// `<cwd>/.mcp.json` and `<cwd>/.igrok/config.toml` trigger a
    ...
    /// `<cwd>/.igrok/`) previously registered for `cwd` via
...
///
/// **Known limitation:** if `<cwd>/.igrok/` does not yet
/// exist at session-open time, the `.igrok/` watch fails ENOENT and is
/// swallowed at `debug!`. A later `mkdir <cwd>/.igrok/` followed by a
/// write to `<cwd>/.igrok/config.toml` will NOT be observed — the
...
/// expected for a freshly-opened session whose `<cwd>/.igrok/` hasn't
...
const VENDOR_CONFIG_ROOT_NAMES: &[&str] = &[".igrok", ".agents", ".claude", ".cursor"];
...
fn project_grok_refresh_dirs(project_root: &Path) -> Vec<(PathBuf, RecursiveMode)> {
    let project_grok = project_root.join(".igrok");
...
mod tests {
    ...
    fn plan_skills_watch_targets_scopes_vendors_and_seeds_refresh() {
        ...
        let project_grok = project.join(".igrok");
    ...
    fn plan_skills_watch_targets_multi_vendor_refresh_fanout() {
        let grok_home = PathBuf::from("/home/u/.igrok");
    ...
    fn plan_skills_watch_targets_seeds_all_missing_project_vendor_roots() {
        let tmp = TempDir::new().unwrap();
        let project = tmp.path();
        let grok_home = project.join("elsewhere").join(".igrok");
        let plan = plan_skills_watch_targets(&[], &grok_home, Some(project));
    ...
    fn plan_skills_watch_targets_partial_vendors_seed_only_missing() {
        ...
        for name in [".igrok", ".agents", ".cursor"] {
            let root = project.join(name);
            expected.push((root.clone(), RecursiveMode::NonRecursive));
            expected.extend(vendor_skill_refresh_dirs(&root));
        }
    ...
    fn workflow_change_classifies_missing_directory_creation() {
        let grok = Path::new("/tmp/project/.igrok");
        assert_eq!(
    ...
    /// A write to `<cwd>/.igrok/config.toml` must surface as
    ...
    fn project_cwd_toml_triggers_reload() {
        ...
        let project_grok = cwd.path().join(".igrok");
        fs::create_dir_all(&project_grok).unwrap();
        ...
        assert!(
            found,
            "expected ProjectConfigChanged for <cwd>/.igrok/config.toml within 2s"
        );
    }
    ...
    /// `start` must light up `<new_cwd>/.igrok/config.toml` writes
    ...
    fn watch_path_dynamic_registration() {
        ...
        let project_grok = new_cwd.path().join(".igrok");
        fs::create_dir_all(&project_grok).unwrap();
        fs::write(project_grok.join("config.toml"), "").unwrap();
