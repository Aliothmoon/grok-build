...
fn session_token_auth_gate_truth_table() {
    ...
    for fp in [false, true] {
        ...
        // Session method: a genuine per-model Byok never refreshes.
        assert!(!gate(true, ModelByok::Byok, fp));
    }
    // [LOCAL-DEV] Session method + NotByok: refresh only against a
    // first-party host — a third-party endpoint must never receive the
    // session token, even when the model declares no key of its own (its
    // credentials may live entirely in extra_headers).
