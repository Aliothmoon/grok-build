...
mod tests {
    ...
    fn toml_claim_survives_when_client_cursor_insert_skipped() {
        let cwd = empty_cwd();
        write_cursor_project_mcp(cwd.path(), "killswitch-cache");
        std::fs::create_dir_all(cwd.path().join(".igrok")).unwrap();
        std::fs::write(
            cwd.path().join(".igrok").join("config.toml"),
            r#"
...
"#,
    ...
    fn load_plugin_mcp_creates_stdio_server_with_env_substitution() {
        ...
        let (servers, _) = load_plugin_mcp_servers_from_config(
            &config,
            "team-tool",
            "/home/user/.igrok/plugins/team-tool",
            "/home/user/.igrok/plugin-data/team-tool",
        );
        ...
        match &servers[0] {
            acp::McpServer::Stdio(acp::McpServerStdio {
                ...
                assert_eq!(
                    args.as_slice(),
                    &["/home/user/.igrok/plugins/team-tool/mcp-echo-server.py"]
                );
            }
