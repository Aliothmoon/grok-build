...
//!   under `~/.igrok/worktree_pool/<instance_id>/` with a `.pid` liveness
...
/// `~/.igrok/worktree_pool/<instance_id>/` with a `.pid` file recording
...
impl WorktreePool {
    ...
    /// `~/.igrok/worktree_pool/<instance_id>/`. A `.pid` file is written so
...
pub struct AdoptableWorktree {
    /// Current path: `~/.igrok/worktree_pool/<old_instance>/<pool_id>/`
...
/// `~/.igrok/worktree_pool/`, reads each `.pid` file, and checks
...
/// The base pool directory under `~/.igrok/`.
...
/// `CleanAll` mode. Placed under `~/.igrok/worktree_pool/<instance_id>/<uuid>/`.
