...
/// Walks from cwd up to the git repo root checking `.igrok/config.toml` at each level.
/// Project-scoped `.igrok/config.toml` entries override global `~/.igrok/config.toml`
...
/// `.igrok/config.toml`, otherwise user (global config, `~/.claude.json`,
...
/// Load MCP servers with project-scoped overrides from `.igrok/config.toml`.
///
/// Merge strategy:
/// 1. Load MCP servers from global `~/.igrok/config.toml`
/// 2. Walk from git repo root down to `cwd`, loading `.igrok/config.toml` at each level
        ...
        if let Ok(root) = crate::config::load_config_file(config_path) {
            ...
            if !project_servers.is_empty() {
                tracing::info!(
                    ...
                    "Loaded project-scoped MCP servers from .igrok/config.toml"
                );
...
/// Upsert an MCP server entry in `~/.igrok/config.toml`.
///
...
/// config file, e.g. a project-scoped `.igrok/config.toml`.
...
/// Delete an MCP server entry from `~/.igrok/config.toml`.
///
...
/// config file, e.g. a project-scoped `.igrok/config.toml`. OAuth credential
    ...
    let message = if !has_command && !has_url {
        format!(
            "`mcp_servers.{name}` has no transport. To run it, set `command = \"...\"` or \
             ...
             See ~/.igrok/docs/user-guide/07-mcp-servers.md"
        )
    } else {
        format!(
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` has an unrecognized field `{field}`; it is \
                             ignored. See ~/.igrok/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                }
                if config.enabled
                    ...
                    problems.push(McpServerConfigProblem {
                        ...
                        message: format!(
                            "`mcp_servers.{name}` is enabled but its `{field}` is blank. \
                             ...
                             off. See ~/.igrok/docs/user-guide/07-mcp-servers.md"
                        ),
                    });
                    continue;
                }
...
/// (`"user"` or `"project"`).
///
/// Overlays project-scoped `.igrok/config.toml` files from `cwd` up to the
...
/// Path to the user-level config file (`~/.igrok/config.toml`).
pub fn user_config_path() -> PathBuf {
    config_path()
}

/// Path to a project-level config file (`<dir>/.igrok/config.toml`).
pub fn project_config_path(dir: &std::path::Path) -> PathBuf {
    dir.join(".igrok").join("config.toml")
}
...
/// Overrides `[cli] session_registry`; usable before `~/.igrok/config.toml` exists.
...
"#;
    ...
    fn skills_config_parses_paths_and_ignore() {
        let root = toml::from_str::<TomlValue>(
            r#"
[skills]
paths = ["~/.igrok/skills", "~/.igrok/skills/special/SKILL.md"]
ignore = ["~/.igrok/skills/noisy/SKILL.md"]
"#,
        )
        .unwrap();
        ...
        assert_eq!(
            cfg.paths,
            vec!["~/.igrok/skills", "~/.igrok/skills/special/SKILL.md"]
        );
        assert_eq!(cfg.ignore, vec!["~/.igrok/skills/noisy/SKILL.md"]);
    }
...
"#,
    ...
    async fn enable_unstick_only_touches_nearest_project_definition() {
        ...
        std::fs::create_dir_all(nested.join(".igrok")).unwrap();
...
enabled = false
"#;
        let ancestor = tmp.path().join(".igrok").join("config.toml");
        let nearer = nested.join(".igrok").join("config.toml");
