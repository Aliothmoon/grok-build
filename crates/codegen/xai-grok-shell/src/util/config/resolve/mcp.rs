...
/// | config       | `[features] mcp_liveness_watchers` in `~/.igrok/config.toml`     |
...
/// | config       | `[features] mcp_auto_restart` in `~/.igrok/config.toml`          |
...
/// | config       | `[features] mcp_push_server_status` in `~/.igrok/config.toml`    |
...
/// | config       | `[features] mcp_recursive_config_watch` in `~/.igrok/config.toml`    |
...
/// `.igrok/config.toml` chain (`cwd` → git root), deepest file wins.
///
...
mod max_mcp_output_bytes_tests {
    ...
    fn project_chain_deepest_file_wins() {
        ...
        std::fs::create_dir_all(sub.join(".igrok")).unwrap();
        std::fs::create_dir_all(root.join(".igrok")).unwrap();
        std::fs::write(
            root.join(".igrok/config.toml"),
        ...
        std::fs::write(sub.join(".igrok/config.toml"), "[ui]\nvim_mode = true\n").unwrap();
        assert_eq!(super::project_max_mcp_output_bytes(&sub), Some(30_000));

        // No .grok files with the key anywhere → None.
        std::fs::remove_file(root.join(".igrok/config.toml")).unwrap();
