...
//!   firehose to `~/.igrok/debug/<session_id>.txt` (one file per session), with a
...
pub(crate) enum DebugTarget {
    /// `GROK_DEBUG_LOG=1` → route per session into `<dir>` (`~/.igrok/debug`).
...
/// per session into `~/.igrok/debug`, an explicit path writes a single file.
///
...
/// `~/.igrok/debug` older than [`LOG_RETENTION`] so the dir doesn't grow
