...
//! (release builds, self-extracted to `~/.igrok/vendor/<name>-<ver>-<target>`) →
//! `~/.igrok/vendor/{name}` if a regular file → `which` on the agent `$PATH`.
...
// (release pipeline). Self-extracted to `~/.igrok/vendor` on first use, mirroring
...
/// Write embedded `bytes` to `~/.igrok/vendor/<versioned_name>` (chmod 755) on
...
/// `~/.igrok/vendor/<bin>` → `which`. Env and vendor only require `is_file()` here
...
mod tests {
    ...
    /// bytes self-extract under `~/.igrok/vendor` and the extracted `bfs` runs.
