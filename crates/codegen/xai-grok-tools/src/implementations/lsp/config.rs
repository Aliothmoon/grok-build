//! LSP server configuration from `.igrok/lsp.json`.
...
pub fn load_servers_with_plugins_sourced(
    ...
    let project_path = cwd.join(".igrok").join("lsp.json");
...
/// Load LSP server configs from `~/.igrok/lsp.json` and `<cwd>/.igrok/lsp.json`.
