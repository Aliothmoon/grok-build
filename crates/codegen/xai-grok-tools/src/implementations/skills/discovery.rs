...
///
/// `skills` is the standard layout (`.igrok/skills/`, `.claude/skills/`,
...
/// name (e.g. `~/.igrok/skills/shell`) is NOT dropped — only skills physically
...
/// `cwd` (exclusive). At each directory, checks for `.igrok/skills/`,
...
pub fn discover_skills_for_paths(
    ...
    let mut config_dir_names: Vec<&str> = vec![".igrok", ".agents"];
...
";
    ...
    fn is_vendor_default_skill_spares_user_skill_outside_vendor_dir() {
        // A user's own "shell" skill in ~/.igrok is NOT a vendor builtin.
        assert!(!is_vendor_default_skill(
            "/home/u/.igrok/skills/shell/SKILL.md",
            "shell"
        ));
    }
    ...
    fn parse_skill_files_drops_denylisted_cursor_skill() {
        ...
        .unwrap();
        // Same name under /.igrok/ → kept (user content).
        let grok_shell = tmp.path().join(".igrok").join("skills").join("shell");
        ...
        assert!(skills[0].path.contains("/.igrok/"));
    }
    ...
    fn discover_skills_for_paths_gates_claude_dir() {
        ...
        .unwrap();
        let grok_skill = sub.join(".igrok").join("skills").join("grok-dyn");
