...
//! after every tool call to check for SKILL.md files in `.igrok/skills/`,
...
/// Directories that contain skill definitions (`.igrok/skills/`, `.agents/skills/`,
/// `.claude/skills/`, `.cursor/skills/`). Shared between startup skill discovery
/// and runtime `SkillDiscoveryReminder`.
pub const SKILL_CONFIG_DIRS: &[&str] = &[".igrok", ".agents", ".claude", ".cursor"];
...
impl SkillDiscoveryReminder {
    ...
    /// (`.igrok/skills/`, `.agents/skills/`, or `.claude/skills/`).
...
impl Reminder for SkillDiscoveryReminder {
    ...
    async fn collect_reminders(
        ...
        // discovery cannot find these because it looks for `.igrok/skills/`
        // sub-directories in *ancestor* dirs, and user-scope skills
        // (~/.igrok/) are outside the git root so the walk breaks early.
