...
pub const GH_RELEASE_REPO: &str = "Aliothmoon/grok-build";
...
pub struct UpdateConfig {
    ...
    /// Auth scope key for `~/.igrok/auth.json`.
...
/// `~/.igrok/bin/grok` symlink target (`../downloads/grok-<version>-<platform>`)
...
/// link whose target binary was deleted (e.g. manual `~/.igrok/downloads`
...
/// Read the cached stable version from `~/.igrok/version.json` (sync, for display).
///
...
/// `~/.igrok/version.json` (written by the auto-updater):
