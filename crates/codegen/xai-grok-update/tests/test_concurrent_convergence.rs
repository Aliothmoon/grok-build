...
/// Assert the active `~/.igrok/bin/grok` resolves to the expected versioned
...
// installers that actually maintain the managed `~/.igrok/bin/grok` symlink
...
async fn disk_probe_rejects_dangling_symlink() {
    ...
    // ~/.igrok/downloads cleanup), the probe must report None — otherwise
