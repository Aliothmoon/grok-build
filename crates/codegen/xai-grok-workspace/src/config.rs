...
pub struct WorkspaceConfig {
    ...
    /// Global hook sources (e.g. `~/.claude/settings.json`, `~/.igrok/hooks/`).
    pub hook_global_sources: Vec<HookSourceConfig>,
    /// Project-scoped hook sources (e.g. `<project>/.igrok/hooks/`).
    ...
    /// `<cwd>/.igrok/lsp.json`: `false` drops them at load, `true` keeps them. The
...
pub enum HookSourceConfig {
    ...
    /// A directory of `*.json` hook files (e.g. `~/.igrok/hooks/`).
    Directory(PathBuf),
}
