...
/// Directory (under `<cwd>/.igrok`) holding every session's checkpoint store.
...
pub(crate) struct CheckpointStore {
    /// Per-session store directory: `<cwd>/.igrok/rewind-checkpoints/<session_id>`.
    dir: PathBuf,
