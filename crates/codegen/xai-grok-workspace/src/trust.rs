...
//!
//! Persists per-folder trust decisions to `~/.igrok/trusted_folders.toml`.
...
//! home that resolves to `None` (rather than a cwd-relative `./.igrok`) when
...
//! a `./.igrok/trusted_folders.toml` that self-trusts its own checkout (fail
//! closed).
...
/// Filename of the folder-trust store under `~/.igrok/`.
...
impl TrustStore {
    ...
    /// `./.igrok` — that fallback would let an untrusted cloned repo's `.grok`
...
/// ROOT (via the `~/.igrok/worktrees.db` registry), so every `grok -w` worktree
...
/// sharing `~/.igrok/`) across the whole read-modify-write so updates merge
...
///
/// The legacy `~/.igrok/trusted-hook-projects` file listed one canonical project
...
mod tests {
    ...
    fn default_path_in_maps_home_and_preserves_no_home() {
        ...
        let home = PathBuf::from("/home/alice/.igrok");
        assert_eq!(
        ...
        // cwd-relative `./.igrok` that grok_home() would invent, which is exactly
        // how a cloned repo's own `<repo>/.igrok/trusted_folders.toml` could
        // masquerade as the user-global store and self-trust the checkout.
